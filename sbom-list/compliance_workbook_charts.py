...
**Optional Jira breached-fix latency** (Slack / PNG subtitles: avg resolution age, then avg / median /
90th percentile **days past SLA** among breached fixes with full SLA math): when ``JIRA_BASE_URL``,
``JIRA_EMAIL``, and ``JIRA_API_TOKEN`` are set and ``COMPLIANCE_CHARTS_SKIP_JIRA`` is not truthy, the script
pulls resolved issues like ``jira_compliance_daily.py``
(see ``jira_metrics_queries.json`` ``base_scope``); resolutions **on or after ``DATE_START``** (env ``DATE_START``,
default ``2026-02-03`` — same inclusive start as the chart X-axis).
Uses ``JIRA_COMPLIANCE_TIMEZONE`` (default UTC), optional ``JIRA_COMPLIANCE_PROJECTS`` / ``ONLY_PROJECT_SHEETS``,
and optional ``JIRA_METRICS_JSON``. By default ``COMPLIANCE_CHARTS_JIRA_RESOLVED_END`` is ``sheet``: resolved issues
are counted only through the **latest date present in the Sheet** so Jira breach totals match portfolio cumulative
coverage (use ``today`` to include fixes after the sheet was last refreshed). Failures are non-fatal.

**Sheet cumulative breached vs Jira (SLA math):** the Sheet’s ``Fixed-Breached`` / cumulative comes from Apps Script
using SLA date math **plus** ``sla-breached`` label when fields are missing; this script’s Jira overlay uses
**calendar math only**. Small gaps can also come from ``dedupe_dates`` (one row per Project+Date).

Depends: pandas, gspread, google-auth, matplotlib, numpy; slack_sdk for Slack chart upload + Block Kit summary.
requests required only when Jira overlay env vars are set.
"""
...
class JiraBreachOverlay(NamedTuple):
    """Jira breach latency metrics plus the resolved-date window used (for Slack/PNG labels)."""
...
# First calendar date shown on charts (YYYY-MM-DD). Same inclusive lower bound for **Jira breach
# cohort** averages (resolved ≥ DATE_START). Override with env ``DATE_START``.
...
# Inclusive resolved-date lower bound for Jira breach averages — **same as DATE_START** (this script).
JIRA_BREACH_METRICS_RESOLVED_START = DATE_START


def _jira_breach_slack_lines(ov: JiraBreachOverlay) -> List[str]:
    """Slack mrkdwn: breached latency — mean intro→resolve, then mean/median/p90 days past SLA."""
    jb = ov.metrics
    if (
        jb.avg_resolution_age_days_breached is None
        or jb.avg_days_past_sla_at_resolution is None
        or jb.median_days_past_sla_at_resolution is None
        or jb.p90_days_past_sla_at_resolution is None
    ):
        return []
    return [
        f"*Avg resolution age (breached):* {jb.avg_resolution_age_days_breached:.1f} days "
        "(Resolved − Introduced)",
        f"*Avg days past SLA (breached):* {jb.avg_days_past_sla_at_resolution:.1f} days "
        "(Resolved − Due)",
        f"*Median days past SLA (breached):* {jb.median_days_past_sla_at_resolution:.1f} days "
        "(typical issue, Resolved − Due)",
        f"*90th percentile days past SLA (breached):* {jb.p90_days_past_sla_at_resolution:.1f} days "
        "(slow tail, Resolved − Due)",
    ]


def _jira_breach_figure_subtitle(ov: JiraBreachOverlay) -> Optional[str]:
    """Subtitle: avg intro→resolve, then avg/median/p90 days past SLA (calendar days)."""
    jb = ov.metrics
    if (
        jb.avg_resolution_age_days_breached is None
        or jb.avg_days_past_sla_at_resolution is None
    ...
    return (
        f"Avg resolution age (breached): {jb.avg_resolution_age_days_breached:.1f} days "
        "(Resolved − Introduced)\n"
        f"Avg days past SLA (breached): {jb.avg_days_past_sla_at_resolution:.1f} days (Resolved − Due)\n"
        f"Median days past SLA (breached): {jb.median_days_past_sla_at_resolution:.1f} days (Resolved − Due)\n"
        f"90th pct days past SLA (breached): {jb.p90_days_past_sla_at_resolution:.1f} days (Resolved − Due)"
    )


def _jira_breach_panel_caption_lines(ov: JiraBreachOverlay) -> List[str]:
    """Panel footnotes: same metrics as Slack/figure."""
    jb = ov.metrics
    if (
        jb.avg_resolution_age_days_breached is None
        or jb.avg_days_past_sla_at_resolution is None
...
def try_fetch_jira_breach_latency_metrics(
    *,
    ...
    """
    Pull resolved issues from Jira (SLA breach = **calendar math** only — ``sla-breached``
    label is ignored) and compute portfolio breach latency summaries (means + median / p90
    of days past SLA) for chart/Slack overlays.

    Resolved **end** date defaults to **today** or, when ``COMPLIANCE_CHARTS_JIRA_RESOLVED_END``
    is ``sheet`` (default), the **minimum** of today and ``sheet_resolved_end_inclusive``
    so Jira counts align with the portfolio row built from the Sheet (same calendar coverage).
    Use ``COMPLIANCE_CHARTS_JIRA_RESOLVED_END=today`` to always run through today.
    """
    _load_dotenv_if_present()
    skip = os.environ.get("COMPLIANCE_CHARTS_SKIP_JIRA", "").strip().lower()
    if skip in ("1", "true", "yes"):
        print(
            "Note: Jira breach latency overlay disabled (COMPLIANCE_CHARTS_SKIP_JIRA).",
            flush=True,
        )
        return None
    ...
    if not base or not email or not token:
        print(
            "Note: Jira breach latency overlay skipped — set JIRA_BASE_URL, JIRA_EMAIL, "
            "and JIRA_API_TOKEN in the environment (or .env). Charts still use the Sheet only.",
            flush=True,
        )
        return None
    ...
    try:
        ...
        print(
            "Warning: zoneinfo unavailable; skip Jira breach latency overlay.",
            file=sys.stderr,
        )
        return None
    ...
    try:
        ...
        print(
            f"Warning: invalid DATE_START / Jira cohort lower bound "
            f"{JIRA_BREACH_METRICS_RESOLVED_START!r}; skip Jira metrics.",
            file=sys.stderr,
        )
        return None
    ...
    if end_mode in ("today", "now"):
        ...
        if end_d < today_d:
            print(
                f"Note: Jira breach cohort uses latest resolved date {end_d} (Google Sheet max date) "
                f"so breach counts line up with portfolio cumulative; today in {tz_name} is {today_d}. "
                f"Set COMPLIANCE_CHARTS_JIRA_RESOLVED_END=today to include newer Jira resolutions.",
                flush=True,
            )
    else:
        end_d = today_d
    ...
    if end_d < start_d:
        print(
            "Warning: Jira resolved cohort end is before DATE_START; skip Jira metrics.",
            file=sys.stderr,
        )
        return None
    ...
    try:
        ...
        print(
            f"Warning: Jira dependencies missing ({e}); skip breach latency overlay.",
            file=sys.stderr,
        )
        return None
    ...
    try:
        ...
        if not base_scope:
            print(
                f"Warning: base_scope missing in {mj}; skip Jira metrics.",
                file=sys.stderr,
            )
            return None
    except OSError as e:
    ...
    try:
        max_results = int(os.environ.get("COMPLIANCE_CHARTS_JIRA_MAX_RESULTS", "100").strip() or "100")
    except ValueError:
        print(
            "Warning: invalid COMPLIANCE_CHARTS_JIRA_MAX_RESULTS; skip Jira metrics.",
            file=sys.stderr,
        )
        return None
    ...
    try:
        ...
        if pair is None:
            print(
                ...
                "breached-fix averages not shown. Set JIRA_FIELD_SEVERITY or "
                "JIRA_FIELD_VULNERABILITY_INTRODUCED if needed.",
                file=sys.stderr,
                flush=True,
            )
            return None
        ...
        if len(issues) == 0:
            print(
                "Hint: If this stays 0 in Navigator too, widen filters or dates; "
                "otherwise check token scope.\n"
                f"JQL:\n{jql}",
                file=sys.stderr,
                flush=True,
            )
        ...
        print(f"Warning: Jira API error ({e}); skip breach latency overlay.", file=sys.stderr)
        return None
...
def plot_portfolio_merged_png(
    ...
    *,
    ...
    if jira_overlay is not None:
        ax_ij_caption_lines.extend(_jira_breach_panel_caption_lines(jira_overlay))
    ax_ij.text(
        0.01,
        -0.072,
        "\n".join(ax_ij_caption_lines),
    ...
    fig.tight_layout(
        ...
        h_pad=1.05,
        w_pad=1.05,
        rect=[0.02, 0.09, 0.98, 0.91],
    )
    fig.suptitle(title, fontsize=14, fontweight="bold", color="#1a1a2e", y=0.982)
    jira_sub = (
        _jira_breach_figure_subtitle(jira_overlay)
        if jira_overlay is not None
        else None
    )
    if jira_sub:
...
def main() -> None:
    ...
    if jira_overlay is not None:
        jb = jira_overlay.metrics
        if (
            jb.avg_resolution_age_days_breached is not None
            and jb.avg_days_past_sla_at_resolution is not None
            and jb.median_days_past_sla_at_resolution is not None
            and jb.p90_days_past_sla_at_resolution is not None
        ):
            print(
                json.dumps(
                    {
                        "jira_resolved_cohort_start": jira_overlay.resolved_start_inclusive,
                        "jira_resolved_cohort_end": jira_overlay.resolved_end_inclusive,
                        "breached_fixes_sla_math_only": jb.breached_issue_count,
                        "avg_resolution_age_days_breached": jb.avg_resolution_age_days_breached,
                        "avg_days_past_sla_at_resolution": jb.avg_days_past_sla_at_resolution,
                        "median_days_past_sla_at_resolution": jb.median_days_past_sla_at_resolution,
                        "p90_days_past_sla_at_resolution": jb.p90_days_past_sla_at_resolution,
                    },
                    indent=2,
                ),
                flush=True,
    ...
    plot_portfolio_merged_png(
        portfolio, out, CHART_TITLE, jira_overlay=jira_overlay
    )
    print(f"Wrote {out}")
