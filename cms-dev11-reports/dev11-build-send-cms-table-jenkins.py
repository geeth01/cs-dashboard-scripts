...
slack_token = 'xoxb-66698'  # Placeholder for Slack Bot Token
...
def is_latest_run_failed(val):
    ...
    if not s or s.lower() == "norun":
        return False
    # Explicit pass markers in the latest cell only (do not use other columns).
    if "✅" in s or re.search(r":white_check_mark:", s, re.IGNORECASE):
        return False
    if re.search(r"\b(success|passed|pass)\b", s, re.IGNORECASE):
        return False
    # Explicit fail in latest cell
    if "❌" in s or re.search(r":x:", s, re.IGNORECASE):
        return True
    # Word-boundary fail words (avoids matching "failures" in e.g. "0 failures" with a pass)
...
def _thread_ts_from_file_share(file_obj, channel_id: str):
    """Parent message ts from a Slack file object's ``shares``."""
    if not file_obj:
        return None
...
def resolve_thread_ts_for_uploaded_file(
    ...
    """Ts of the channel message that carries the uploaded image (thread parent only).

    Retries ``files.info`` briefly because ``shares`` are sometimes filled in after upload,
    then matches the file id on recent ``conversations.history`` messages.
    """
    ...
    for attempt in range(max_retries):
        if last_obj:
            ts = _thread_ts_from_file_share(last_obj, channel_id)
            if ts:
                return ts
        if file_id:
            try:
                info = slack_client.files_info(file=file_id)
                last_obj = info.get("file")
                if last_obj:
                    ts = _thread_ts_from_file_share(last_obj, channel_id)
                    if ts:
                        return ts
    ...
    if file_id:
        try:
            hist = slack_client.conversations_history(channel=channel_id, limit=15)
            for msg in hist.get("messages", []):
                for f_item in msg.get("files") or []:
                    if f_item.get("id") == file_id:
                        return msg.get("ts")
        except SlackApiError:
            pass
    return None
...
def save_to_csv():
 ...
 ############### december 5 ##################
            ...
            # # Write sorted values (oldest → newest)
            # timestamp_strs_sorted = [ts.strftime('%Y-%m-%d %H:%M:%S') for ts in sorted_timestamps]
            # row = [first_line, TIBS] + [val for pair in zip(sorted_statuses, timestamp_strs_sorted) for val in pair]
...
# start_date = '2025-01-24'
# start_date = datetime.today().strftime('%Y-%m-%d') # today's date
start_date = datetime(2026, 4, 9, 16, 23)
...
if team_name == "Unknown":
    team_name = "All"
# print(team_name)
# start_date = datetime(2025, 4, 9, 7, 10)
...
try:
    ...
    if not response.get("ok"):
        ...
        parent_ts = resolve_thread_ts_for_uploaded_file(client, channel_id_gk, response)
        try:
            if parent_ts:
                ...
                print(
                    "Could not resolve thread parent ts for the table image upload; "
                    "not posting the sanities message (avoids a standalone channel post)."
                )
