"""
Dev11 CMS sanity: Slack → CSV → Google Sheets.
...
Usage:
  python dev11-build-send-cms-table-jenkins-unified.py
  python dev11-build-send-cms-table-jenkins-unified.py YYYY-MM-DD
  python dev11-build-send-cms-table-jenkins-unified.py YYYY-MM-DD YYYY-MM-DD
"""
...
def resolve_date_range() -> tuple[datetime, datetime, date, date]:
    ...
    if not start_s or not end_s:
        print(
            ...
            "  python dev11-build-send-cms-table-jenkins-unified.py START [END]",
            file=sys.stderr,
        )
        sys.exit(1)
...
def normalize_suite_name(raw: str) -> str:
    """
    Align Slack suite lines with Google Sheet column A (strip ``dev11,`` prefix, NBSPs,
        ...
        while True:
            ...
            for message in messages:
                ...
                if looks_like_result:
                    ...
                    if lines:
                        # Legacy Live Preview layout: line0 was not the dev11 suite (e.g. bot
                        # prefix); suite on line1, result on line2. New layout: dev11 + Live
                        # Preview on line0, result on line1 — use normal dev11 parsing.
                        lp_legacy_shift = (
                    ...
                    for block in message["blocks"]:
                        if block["type"] in ("section", "header"):
                            ...
                            if not any(ln.strip() for ln in lines_b):
                                continue
                            # Live Preview: old layout had dev11 on line 1, not line 0
                            if (
